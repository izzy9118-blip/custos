...
def test_abandoned_candidate_register_has_no_canonical_effect():
    repo_root = _repo_root()
    ledger = _load_yaml(
        repo_root / "ledgers/identifier-assignment-ledger/LDG-000000001.yaml"
    )

    assert "REG-000000003" not in ledger["entries"]
    assert not (
        repo_root / "registers/candidate-cognitive-component-register"
    ).exists()
...
def test_amendment_assignment_and_ledger_version_are_coherent():
    ...
    version_assignment = _load_yaml(
        repo_root
        / "ledgers/identifier-assignment-ledger/assignments/VER-000000017.yaml"
    )
    version = _load_yaml(repo_root / "records/versions/VER-000000017.yaml")
    ledger = _load_yaml(
        repo_root / "ledgers/identifier-assignment-ledger/LDG-000000001.yaml"
    )
    ...
    assert version["previous_version"] == "1.11"
    assert version["current_version"] == "1.12"
    assert version["source_decision_id"] == "AMD-000000003"
    assert ledger["version"] == "1.12"
    assert ledger["entries"][-2:] == ["AMD-000000003", "VER-000000017"]
